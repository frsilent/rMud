...
//  returns random double number from [0..1] or [0..1)
// ---------------------------------------------------------
...
struct random_percent
{
    ...
    { 
        ...
        // if 1.0 is exclusive, then max incremented so 1.0 never generated.
        if( !inclusive )
            m_max += 1.0;
    }
...
struct normal_generator
{
    ...
    double operator()()
    {
        if(!m_valid) 
        {
            m_rho1 = this->m_generator();
            m_rho2 = this->m_generator();
...
//  returns random double number from [a..b] or [a..b)
// ---------------------------------------------------------
template< bool inclusive, typename generator = random_percent<inclusive> >
struct random_range
{
...
//  returns simulated binomial random number by adding N RNG's
// ---------------------------------------------------------
